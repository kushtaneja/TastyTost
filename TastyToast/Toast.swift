...
// image size
let HRToastImageViewWidth :  CGFloat  = 40.0
let HRToastImageViewHeight:  CGFloat  = 40.0
...
// toastIntention = 0:error , 1:completion, 2:warning
...
extension UIView {
    ...
    func makeToast(message msg: String, toastIntention: Int) {
        let toast = self.viewForMessage(msg, title: nil, image: nil, toastIntention: toastIntention)
        self.showToast(toast!, duration: HRToastDefaultDuration, position: HRToastPositionDefault as AnyObject, toastIntention: toastIntention)
    }
    
    func showToast(_ toast: UIView, duration: Double, position: AnyObject, toastIntention: Int) {
        ...
        toast.layer.borderColor = UIColor.red.cgColor
        
        toast.layer.cornerRadius = HRToastCornerRadius
        
        toast.backgroundColor = UIColor.white
        
        /*
         switch toastIntention {
         case 0 :
         toast.backgroundColor = ColorCode().toastRedColor
         case 1:
         toast.backgroundColor = ColorCode().toastGreenColor
         case 2 :
         toast.backgroundColor = ColorCode().toastBlueColor
         default:
    ...
    func makeToast(message msg: String, duration: Double, yPosition: CGFloat, toastIntention: Int) {
    ...
    /*
    func viewForMessage(_ msg: String?, title: String?, image: UIImage?, toastIntention: Int = 0) -> UIView? {
        ...
        let circleView = AnimatedCircle(frame: CGRect(x: 0.0, y:0.0, width: CGFloat(HRToastImageViewWidth), height:CGFloat(HRToastImageViewHeight)))
        ...
        circleView.layer.cornerRadius = HRToastCornerRadius
        circleView.backgroundColor = UIColor.white
        
        var imageWidth: CGFloat, imageHeight: CGFloat, imageLeft: CGFloat
        imageWidth = circleView.bounds.size.width
        imageHeight = circleView.bounds.size.height
...
//            let maxSizeTitle = CGSize(width: (self.bounds.size.width * HRToastMaxWidth) - imageWidth, height: self.bounds.size.height * HRToastMaxHeight);
            ...
            titleLabel?.layer.cornerRadius = HRToastCornerRadius
            titleLabel?.backgroundColor = UIColor.blue
        }
...
//            msgLabel!.backgroundColor = UIColor.clear
            ...
            let maxSizeMessage = CGSize(width: (self.bounds.size.width) - imageWidth, height: self.bounds.size.height)
            let expectedHeight = msg!.stringHeightWithFontSize(HRToastFontSize, width: maxSizeMessage.width)
            msgLabel!.frame = CGRect(x: imageWidth, y: 0.0, width: (self.bounds.size.width * HRToastMaxWidth) - imageWidth, height: self.bounds.size.height * HRToastMaxHeight)
            msgLabel!.autoresizingMask = ([.flexibleLeftMargin, .flexibleRightMargin, .flexibleTopMargin, .flexibleBottomMargin])
            msgLabel!.layer.cornerRadius = HRToastCornerRadius
            msgLabel!.backgroundColor = UIColor.blue
        }
...
//        let wrapperHeight = max(msgTop + msgHeight + HRToastVerticalMargin, imageHeight + HRToastVerticalMargin * 2)
        ...
        wrapperView.frame = CGRect(x: 0.0, y: 0.0, width: msgWidth + imageWidth, height: imageHeight)
...
//            wrapperView.addSubview(titleLabel!)
        ...
        if msgLabel != nil {
            msgLabel!.frame = CGRect(x: imageWidth, y: 0, width: msgWidth, height: msgHeight)
...
//        }
   ...
   */
    ...
    func viewForMessage(_ msg: String?, title: String?, image: UIImage?, toastIntention: Int = 0) -> UIView? {
        ...
        wrapperView.autoresizingMask = ([.flexibleLeftMargin, .flexibleRightMargin, .flexibleTopMargin, .flexibleBottomMargin])
        wrapperView.layer.cornerRadius = HRToastCornerRadius
        wrapperView.backgroundColor = UIColor.yellow
...
//            }
            ...
            msgLabel!.backgroundColor = UIColor.black
            msgLabel!.alpha = 1.0
            msgLabel!.text = msg
            
            let maxSizeMessage = CGSize(width: (self.bounds.size.width) - circleView.frame.width, height: self.bounds.size.height)
            msgLabel!.frame = CGRect(x: circleView.frame.width, y: 0.0, width: (self.bounds.size.width * HRToastMaxWidth) - circleView.frame.width, height: circleView.frame.height)
//            msgLabel!.autoresizingMask = ([.flexibleLeftMargin, .flexibleRightMargin, .flexibleTopMargin, .flexibleBottomMargin])
            msgLabel!.contentMode = .scaleAspectFit
            msgLabel!.layer.cornerRadius = HRToastCornerRadius
            msgLabel!.layer.masksToBounds = true
            
        }
        wrapperView.frame = CGRect(x: 0.0, y: 0.0, width: msgLabel!.frame.width + circleView.frame.width, height:circleView.frame.height )
        wrapperView.addSubview(circleView)
        wrapperView.addSubview(msgLabel!)
        circleView.animateCircle(duration: 1.0)
